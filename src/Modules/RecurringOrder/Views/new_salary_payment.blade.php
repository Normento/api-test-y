                                                    ...
                                                    <td class="sm-px-24"
                                                        ...
                                                        <p>
                                                            @if (is_null($paymentMethod))
                                                                Le paiement d'un montant total de
                                                                <strong>{{ $payment->total_amount_to_paid }}
                                                                    <sup>F</sup></strong> vient d'être confirmé pour le
                                                                client
                                                                <strong>{{ $client->first_name }}
                                                                    {{ $client->last_name }}</strong>
                                                                pour la prestation du mois de
                                                                <strong>{{ $payment->month_salary }}
                                                                    {{ $payment->year }}</strong> tous frais compris
                                                                dont <strong>{{ $payment->employee_salary_amount }}
                                                                    <sup>F</sup></strong> pour le salaire net de
                                                                l'employé
                                                            ...
                                                            @else
                                                                ...
                                                                <strong>{{ $client->first_name }}
                                                                    {{ $client->last_name }}</strong>
                                                                a payé un montant total de
                                                                <strong>{{ $payment->total_amount_to_paid }}
                                                                    <sup>F</sup></strong> pour la prestation du mois de
                                                                <strong>{{ $payment->month_salary }}
                                                                    {{ $payment->year }}</strong> via
                                                                {{ $paymentMethod == 3 ? 'son portefeuille YLOMI' : ($paymentMethod == 1 ? 'MTN Mobile Money' :($paymentMethod == 4 ? "CARTE BANCAIRE" : 'MOOV Mobile Money')) }},
                                                                tous frais compris dont
                                                                <strong>{{ $payment->employee_salary_amount }}
                                                                    <sup>F</sup></strong> pour le salaire net de
                                                                l'employé
                                                                <strong>{{ $payment->employee->full_name }}</strong>.
                                                            @endif
